...
- **Automatische Einkaufsliste 🛒**: Generate a shopping list automatically from the weekly plan based on meal ingredients.
- **Smarter Rezept-Import per Link 🔗**: Automatically scrape recipe websites (e.g. Chefkoch) to fill in name, image, and ingredients.
- **"Social Feed" mit Freunden 🌍**: A dedicated "Feed" tab to see what friends are cooking and easily copy their meals into your own plan.
- **Statistiken & Insights 📊**: A improved dashboard showing interesting facts like vegetarian percentage, most cooked meals, or neglected recipes.
- **Zeit- und Aufwands-Filter ⏱️**: Add duration and difficulty to meals so the planner can suggest quick meals on weekdays and elaborate ones on weekends.
- **Zutaten-Vorratskammer (Pantry) 🥫**: Track pantry staples so they are automatically checked off the shopping list.
- **Koch-Tagebuch (Bilder-Galerie) 📸**: Instead of a single image, store a history of images and dates every time you cook a meal.
- **Saisonale Vorschläge & Tags ☀️❄️**: Custom tags (e.g. #Summer, #Grill) to let the algorithm prefer seasonal meals.
